...
//type:"excel"或者doc
//name:表名
var tableExport=function(data,head_data,type,name) {
      var defaults = {
        csvSeparator: ',',
        csvEnclosure: '"',
        consoleLog: false,
      ...
      var rowspans = [];
        ...
        for(var i=0;i<data.dataList.length;i++){
            excel += "<tr>";
            for(var m=0;m<head_data.length;m++){
            	excel += "<td style='width:100px;height:80px;border:1px solid #000'>"+(data.dataList[i][head_data[m].value]==null?"":data.dataList[i][head_data[m].value])+"</td>";
        	}
            excel += '</tr>';
      	}
